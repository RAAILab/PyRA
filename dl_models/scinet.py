...
class Interactor(nn.Module):
    def __init__(self, in_planes, splitting=True,
        ...
        if self.kernel_size % 2 == 0:
            pad_l = self.dilation * (self.kernel_size - 2) // 2 + 1  # 디폴트: stride==1
            pad_r = self.dilation * (self.kernel_size) // 2 + 1  # 디폴트: stride==1

        else:
            pad_l = self.dilation * (self.kernel_size - 1) // 2 + 1  # 2번째 계층에 커널 사이즈 3으로 고정
...
class LevelSCINet(nn.Module):
    ...
    def forward(self, x):
        ...
        return x_even_update.permute(0, 2, 1), x_odd_update.permute(0, 2, 1)  # 짝수: B, T, D 홀수: B, T, D
...
class SCINet_Tree(nn.Module):
    ...
    def forward(self, x):
        ...
        # 재귀적으로 재정렬. 이 과정은 ./utils/recursive_demo.py 을 통해 확인 가능.
...
class SCINet(nn.Module):
    def __init__(self, input_seq, output_seq, input_dim=1, hid_size=1, num_stacks=1,
        ...
        if self.single_step_output_One:  # N번째 타임스텝만 반환할 때.
            if self.stacks == 2:
                if self.concat_len:
    ...
    def forward(self, x):
        assert self.input_len % (np.power(2,
                                          self.num_levels)) == 0  # 입력 길이를 균등하게 두 부분으로 나눔. (3레벨 예시: 32 -> 16-> 8-> 4)
        if self.pe:
